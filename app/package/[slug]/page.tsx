import { getPackageBySlug, packageInterface } from '@/actions/packages.actions'
import Footer from '@/components/Footer'
import Navbar from '@/components/Navbar'
import Rating from '@/components/Rating'
import { getLocalHotelImagePath } from '@/components/sections/AccomodationSection'
...
export default async function Page({
    ...
    const resp = await getPackageBySlug({ slug });
    const pkg: packageInterface = resp?.data;
    // console.log('this is package', pkg);
        ...
        <>
            ...
            <main className='flex flex-col gap-y-7 py-7'>
                <div className='px-cont'>
                    <p className='text-sm text-gray-600 items-center gap-1 hidden sm:flex'>Home <LuChevronRight className='inline' /> Package Details <LuChevronRight className='inline' /> {pkg.title}</p>
                </div>
                <section className='flex flex-col gap-y-7 px-cont max-w-[1100px] w-full mx-auto'>
                    <h1 className='border-b border-gray-300 pb-5 mt-10 text-4xl leading-none text-left text-brand-secondary/93 font-bold'>{pkg.title}</h1>
                        ...
                        <div className='grid grid-cols-1 min-[707px]:grid-cols-2 gap-8'>
                            ...
                            <div className='rounded-xl border-2 border-gray-100 p-4'>
                                <div className='relative w-full h-[200px] overflow-hidden rounded-[17px]'>
                                    <Image
                                        alt='hotel thumbnail'
                                        src={getLocalHotelImagePath(pkg.makkahHotel.name) || '/images/hotels/default.png'}
                                        fill
                                        className='object-cover object-center'
                                    />
                                </div>
                                <div className='pt-4 flex-1 flex flex-col'>
                                    <div className='bg-brand-secondary rounded-sm px-5 py-0.5 mb-1 w-fit self-start'>
                                        <span className='text-white'>Makkah ( {pkg.makkahNights} Nighs )</span>
                                    </div>
                                    <div className='flex flex-1 h-full justify-between flex-col gap-y-2 mt-4'>
                                        <h2 className='text-3xl leading-none mb-2 font-bold text-brand-secondary'>{pkg.makkahHotel.name}</h2>
                                        ...
                                        <div className='mx-[7%] mt-2'>
                                            ...
                                            <Link href={'#query-form-section'} className='btn-secondary text-xl w-full'>Inquire Now</Link>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className='rounded-xl border-2 border-gray-100 p-4'>
                                <div className='relative w-full h-[200px] overflow-hidden rounded-[17px]'>
                                    <Image
                                        alt='hotel thumbnail'
                                        src={getLocalHotelImagePath(pkg.madinahHotel.name) || '/images/hotels/default.png'}
                                        fill
                                        className='object-cover object-center'
                                    />
                                </div>
                                <div className='pt-4 flex flex-col'>
                                    <div className='bg-brand-secondary rounded-sm px-5 py-0.5 mb-1 w-fit self-start'>
                                        <span className='text-white'>Madinah ( {pkg.madinahNights} Nights )</span>
                                    </div>
                                    <div className='flex flex-col gap-y-2 mt-4'>
                                        <h2 className='text-3xl leading-none mb-2 font-bold text-brand-secondary'>{pkg.madinahHotel.name}</h2>
                                        <div className='flex items-center gap-5'>
                                            <Rating value={pkg.rating} />
                                            <div className='flex gap-1 text-brand-secondary'>
                                                <FaLocationDot size={24} />
                                                <p className='text-xl font-medium'>Madinah</p>
                                            </div>
                                        </div>
                                        <div className='mt-3 mb-5 flex items-center gap-3'>
                                            <Image
                                                ...
                                                height={25}
                                            />
                                            <p className='leading-none text-lg font-light'>2.95km from Haram (Masjid E Nabvi)</p>
                                        </div>
                                        <div className='mx-[7%] mt-2'>
                                            <div className='mb-5 h-[2px] bg-brand-secondary-300' />
                                            <Link href={'#query-form-section'} className='btn-secondary text-xl w-full'>Inquire Now</Link>
                    ...
                    <div className='px-4 py-6 sm:px-7 rounded-xl border-2 border-gray-200'>
                        ...
                        <div className='flex flex-wrap items-center justify-evenly gap-5 sm:gap-10'>
                    ...
                    <div className='grid grid-cols-1 min-[707px]:grid-cols-2 gap-8'>
                        <div className='flex flex-col'>
                            ...
                            <Link href={'#query-form-section'} className='btn-secondary text-2xl w-full'>Book Now</Link>
                        </div>
